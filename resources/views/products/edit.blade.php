    ...
    <div class="container">
        ...
        <form id="productForm" action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3 col-6">
                    ...
                    <select name="category_id" class="form-control" id="category_id" required>
                        ...
                        @foreach ($categories as $key => $item)
                            <option {{ isset($edit) && $key == $product->category_id ? 'selected=selected' : '' }}
                    ...
                    <select name="sub_category_id" class="form-control" id="sub_category_id" required>
                        ...
                        @if (isset($edit) && $edit)
                            @foreach ($sub_categories as $key => $item)
                                <option {{ isset($edit) && $key == $product->sub_category_id ? 'selected=selected' : '' }}
                                    value="{{ $key }}">{{ $item }}</option>
                            @endforeach
                        @endif
                    </select>
                ...
                @foreach ($productSize as $key => $item)
                    <div class="row sizeDiV">
                        <div class="mb-3 col-6">
                            ...
                            <input type="text" class="form-control" value="{{ $item->size }}" id="size"
                                name="size[]" required>
                        </div>
                        <div class="mb-3 col-5">
                            <label for="name" class="form-label">Price:</label>
                            <input type="number" class="form-control" value="{{ $item->price }}" id="price"
                                name="price[]" required>
                        </div>
                        @if($key == 0)
                        ...
                        <div class="mb-3 col-1">
                            <input type="hidden" value="{{ $item->id }}" name="product_size_id[]">
                            <button type="button" class="btn deleteBtn form-control btn-danger"
            ...
            <div class="mb-3">
                ...
                <input type="file" class="form-control" id="images" name="images[]" multiple {{ isset($edit) && $edit ? '' : 'required' }}>
            </div>
            <div class="row">
                @if (isset($edit) && $edit)
                    @foreach ($productImage as $key => $item)
                        <div class="col-md-3 img_{{ $item->id }}">
                            ...
                            <button type="button" onClick="deleteImage({{ $item->id }})"
                                class="btn deleteImage form-control btn-danger" style="margin-top: 10px;">Delete</button>
                        </div>
                    @endforeach
                @endif
            </div>
            <button type="submit" class="btn btn-primary"
                style="margin-top: 20px;">{{ isset($edit) && $edit ? 'Update' : 'Create' }} Product</button>
        </form>
    </div>
@endsection
...
<script>
    ...
    function deleteImage(productsId) {
            var confirmDelete = confirm('Are you sure you want to delete this Image?');
            if (confirmDelete) {
                $.ajax({
                    type: 'DELETE',
                    url: '/products-image/' + productsId,
                    success: function(response) {
                        console.log(response);
                        $('.img_' + productsId).remove();
                    },
                    error: function(xhr) {
